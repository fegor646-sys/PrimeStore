// Стартовий асортимент магазину. Використовується ОДИН раз - під час першого
// запуску, щоб наповнити таблицю shop_items. Далі єдине джерело правди - база,
// а весь асортимент редагує адмін через CMS (/admin.html -> Товари).
//
// kind визначає поведінку картки:
//   simple  - звичайний товар, фіксована ціна (мерч, VIP, магніт...)
//   slot    - прокрут PRIME Mystery Slot
//   amount  - товар із введенням кількості (фішки: 1 PC = 1 грн)
//   variant - товар із вибором номіналу (турнірні квитки)
//
// variants зберігається в полі meta як JSON: [{ label, price }]
