...
app.post("/api/admin/orders/:id/status", requireAdmin, (req, res) => {
  ...
  const text =
    req.body.status === "done"
      ? `✅ Заказ #${result.order.id} выполнен: <b>${tg.escapeHtml(result.order.itemTitle)}</b>`
      : req.body.status === "canceled"
      ? `🔴 Заказ #${result.order.id} отменён. На баланс возвращено <b>${result.order.price} PC</b>.`
      : null;
...
app.post("/api/admin/coin-requests/:id/resolve", requireAdmin, (req, res) => {
  ...
  const text =
    status === "approved"
      ? `💰 Оплата подтверждена. Начислено <b>${result.request.amount} Prime Coin</b>.`
      : `❌ Заявка на покупку Prime Coin отклонена.`;
...
app.post("/api/admin/achievements/:id/resolve", requireAdmin, (req, res) => {
  ...
  const result = db.resolveAchievement(req.params.id, {
    ...
    title: req.body.title || "Достижение подтверждено",
  });
  if (!result.ok) return res.status(400).json({ success: false, ...result });

  const text =
    status === "approved"
      ? `🏆 Достижение подтверждено. Начислено <b>${result.awarded} Prime Coin</b>.`
      : `❌ Заявка на достижение отклонена.`;
...
app.post("/api/admin/balance", requireAdmin, (req, res) => {
  ...
  if (mode === "rake" || mode === "deposit") {
    ...
    tg.notifyUser(
      targetUserId,
      `💎 Начислено <b>${result.coins} Prime Coin</b> — ${mode === "rake" ? "за рейк" : "за депозит"} (${result.percent}% от ${result.base} грн).`
    ).catch((e) => console.error(e));
    return res.json({ success: true, coins: result.coins, profile: db.getProfile(targetUserId) });
  }

  const amount = Math.round(Number(value) || 0);
  const result = db.applyBalance(targetUserId, amount, reason || (amount > 0 ? "Начисление администратором" : "Списание администратором"));
  if (!result.ok) return res.status(400).json({ success: false, ...result });
  if (amount > 0) {
    tg.notifyUser(targetUserId, `💎 Начислено <b>${amount} Prime Coin</b>.`).catch((e) => console.error(e));
  }
...
app.listen(PORT, () => console.log(`PRIME STORE запущен на порту ${PORT}`));
