...
const app = express();
app.use(cors());
// Скриншоти досягнень приходять як base64 усередині JSON, тому ліміт вищий за типовий.
...
// Один запит віддає все, що потрібно головному екрану — менше round-trip'ів
...
/* ------------------------------------------------------------------ */
// Зображення лежать файлами в public/uploads, а в базі зберігається лише шлях.
// Так /api/state залишається легким — інакше кожне завантаження застосунку
// тягнуло б усі зображення асортименту в base64.
const UPLOAD_DIR = path.join(__dirname, "public", "uploads");
fs.mkdirSync(UPLOAD_DIR, { recursive: true });

// Видаляємо лише те, що лежить у нашій папці завантажень: шлях із сіду
// (assets/items/...) або зовнішнє посилання чіпати не можна.
...
app.put("/api/admin/items/:id", requireAdmin, (req, res) => {
  ...
  // Зображення замінили — старий файл більше нікому не потрібен.
...
// Ручне нарахування: і «+N Prime Coin просто так», і відсоток від рейка/депозиту.
