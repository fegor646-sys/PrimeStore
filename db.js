...
function seedShopIfNeeded() {
  ...
  console.log(`[db] Завантажено товарів: ${SHOP_SEED.length}`);
}
...
function seedEarnRulesIfNeeded() {
  ...
  console.log(`[db] Завантажено правил нарахування: ${EARN_SEED.length}`);
}
...
// Старі бази могли бути створені до появи порядку розділів.
...
// Єдина точка зміни балансу: будь-який рух Prime Coin записується в
// історію операцій, тому «Історія» і профіль завжди збігаються з балансом.
const applyBalance = db.transaction((telegramId, amount, reason, ref = {}) => {
  ...
  // Повернення за скасоване замовлення — це не заробіток, а відкат витрати:
  // інакше «всього зароблено» у профілі роздувалося б на кожному скасуванні.
...
/* ------------------------------------------------------------------ */
/* Магазин: акції + розрахунок актуальної ціни                         */
/* ------------------------------------------------------------------ */
// Акція активна, лише якщо задано ціну і поточний момент потрапляє у вікно дат.
// Порожня дата = «без обмеження» з цього боку. Нічого чистити за розкладом
// не треба: щойно вікно закрилося, ціна сама повертається до звичайної.
...
const purchase = db.transaction((telegramId, itemId, options = {}) => {
  ...
  if (item.kind === "variant") {
    ...
    // Знижка застосовується пропорційно: акція на товар працює і для номіналів.
...
// Скасування замовлення повертає Prime Coin назад — окремим рядком в історії,
// щоб гравець бачив і списання, і повернення.
...
/* ------------------------------------------------------------------ */
/* Правила нарахування (довідник «Отримати»)                           */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Автонарахування за рейк і депозит                                   */
/* ------------------------------------------------------------------ */
